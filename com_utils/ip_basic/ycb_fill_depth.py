...
from ope_utils import pcl_utils, visualization_utils
...
def get_one_show(nid):
    ...
    pcld, choose = pcl_utils.dpt2cld(
        data['depth'], data['obj_info_lst'][0]['K'], cam_scale
    )
    nrm = my_utils.get_normal(pcld)
    nrm_map = visualization_utils.get_normal_map(nrm, choose)
    ...
    pcld, choose = pcl_utils.dpt2cld(
        final_dpt, data['obj_info_lst'][0]['K'], scale_2_80
    )
    nrm = my_utils.get_normal(pcld)
    nrm_map_final = visualization_utils.get_normal_map(nrm, choose)
