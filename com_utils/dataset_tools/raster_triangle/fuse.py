...
def collect_train_info(cls_name):
    ...
    train_fns = file_utils.read_lines(tr_pth)
...
def collect_linemod_set_info(
    ...
    file_utils.save_pickle(
        os.path.join(cache_dir, '{}_info.pkl').format(linemod_cls_name), database
    )
    return database
...
def randomly_read_background(background_dir,cache_dir):
    if os.path.exists(os.path.join(cache_dir,'background_info.pkl')):
        fns = file_utils.read_pickle(os.path.join(cache_dir,'background_info.pkl'))
    else:
        fns = glob(os.path.join(background_dir,'*.jpg')) + \
            glob(os.path.join(background_dir,'*.png'))
        file_utils.save_pickle(os.path.join(cache_dir,'background_info.pkl'), fns)
