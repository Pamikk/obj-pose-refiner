...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        return new_points
class PointFeatLayer(nn.Module):
    def __init__(self, use_normal = True):
        super(PointFeatLayer, self).__init__()
        in_channel = 3
        self.sa1 = PointNetSetAbstraction(npoint = 2000, radius = 0.1, nsample = 32, in_channel = in_channel, mlp = [16, 16, 32], group_all = False)
        # self.sa2 = PointNetSetAbstraction(npoint = 10000, radius = 0.1, nsample = 8, in_channel = in_channel, mlp = [16, 16, 32], group_all = False)

    def forward(self, pts):
        B, _, _, = pts.shape
        _, output_pts = self.sa1(pts,None)
